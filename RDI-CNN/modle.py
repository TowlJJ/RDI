...
class MWBlock1(nn.Module):
    def __init__(self):
        ...
        self.conv3 = ConvFusion(64)
        self.conv = nn.Conv2d(192, 64, kernel_size=1, stride=1, padding=0)
        ...
        self.linear_layer = nn.Linear(3, 128)
...
class MWBlock2(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv2d(512, 128, kernel_size=1, stride=1, padding=0)
        self.layer = nn.Sequential(
            SpecialBlock(128, 256, [2, 1]),
            CommonBlock(256, 256, 1),
            CommonBlock(256, 256, 1),
        ...
        self.soft = nn.Softmax(dim=1)
        self.linear_layer = nn.Linear(4, 256)
        
    def forward(self, x1, x2, x3, x4):
        ini_x = torch.cat((x1, x2, x3, x4), dim = 1)
        ini_x = self.conv(ini_x)
        ini_x = self.layer(ini_x)
...
class MWBlock3(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv2d(1024, 256, kernel_size=1, stride=1, padding=0)
        self.soft = nn.Softmax(dim=1)
        self.linear_layer = nn.Linear(4, 256)
