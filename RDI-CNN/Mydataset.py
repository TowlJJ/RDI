...
class RDataset(Dataset):
    def __init__(self, root, transform, step):
        self.root = root
        self.images = []
        self.transform = transform
        self.labels = {}
        ...
        classes_folder = os.path.join(self.root, self.step)
        classes = [i for i in os.listdir(classes_folder) if os.path.isdir(os.path.join(classes_folder, i))]
        self.class_to_idx = {'real': 0, 'fake': 1}  # 假设真实标签为0，假标签为1
        ...
        for index_path in images_path:
            ...
            label = 0 if folders[-1] == 'real' else 1
            self.images.append(index_path)
            self.labels[index_path] = label
...
class dDataset(Dataset):
    def __init__(self, R_root, D_root, transform, step):#‘train’、‘test’
        self.R_root = R_root
        self.D_root = D_root
        self.images = []
        self.transform = transform
        ...
        classes_folder = os.path.join(self.R_root, self.step)
        classes = [i for i in os.listdir(classes_folder) if os.path.isdir(os.path.join(classes_folder, i))]
        self.class_to_idx = {'real': 0, 'fake': 1}  # 假设真实标签为0，假标签为1
